...
// Coordina Scout -> Trends -> Competitive -> Intelligence Core -> Notify -> Brief → Trends → Competitive Intel → Notify en secuencia unificada
...
export default async function handler(req, res) {
  ...
  // ── FASE 4: Intelligence Core (fire-and-forget) ──────────────────────────────
  // Corre async en background (~60s). Guarda analisis en Supabase + Telegram.
  if (SB_URL && SB_KEY) {
    try {
      fetch(BASE + '/api/intelligence', {
        headers: {
          'x-claude-key': CLAUDE, 'x-supabase-url': SB_URL, 'x-supabase-key': SB_KEY,
          'x-tg-token': TG_TOKEN || '', 'x-tg-chat': TG_CHAT || ''
        }
      }).catch(() => {});
      results.agents['intelligence'] = { success: true, status: 'triggered_async' };
    } catch(e) { results.agents['intelligence'] = { success: false, error: e.message }; }
  }
